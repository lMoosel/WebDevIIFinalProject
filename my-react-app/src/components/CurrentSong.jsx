...
export function CurrentSong(props) {
  ...
  const { data, loading, error, refetch } = useQuery(
    queries.GET_SPOTIFY_CURRENTLY_PLAYING,
    {
      variables: {
        id: user._id,
      },
  ...
  return (
    <div id="Current-song-div">
      ...
      {!loading && !error && !data && (
        <p>Unable to fetch authentication URL, please try again later.</p>
      )}
      {data && data.getSpotifyCurrentlyPlaying && (
        <>
          <div id="cs-div">
            <h1 id="current-song">
              <Link to={`/track/${data.getSpotifyCurrentlyPlaying.item.id}`}>
                {data.getSpotifyCurrentlyPlaying.item.name}
              </Link>{" "}
              by{" "}
              {data.getSpotifyCurrentlyPlaying.item.artists.map(
                (artist, index) => (
                  <span key={index}>
                    {index > 0 && ", "}
