...
export function Artist(props) {
  ...
  const {
    ...
    error: artistError,
  } = useQuery(queries.GET_SPOTIFY_ARTIST, {
    variables: {
      id: user._id,
      artistId: artistid,
    },
  ...
  const {
    ...
    error: relatedError,
  } = useQuery(queries.GET_RELATED_ARTISTS, {
    variables: {
      id: user._id,
      artistId: artistid,
    },
  ...
  const {
    ...
    loading: topLoading,
    error: topError,
  } = useQuery(queries.GET_ARTIST_TOP, {
    variables: {
      id: user._id,
      artistId: artistid,
    },
  ...
  if (artistError || relatedError || topError)
    ...
    <div id="Genre-info-div">
      ...
      {related.map((otherArtist) => (
        <div key={otherArtist.id}>
          <p>
            <Link to={`/artist/${otherArtist.id}`}>{otherArtist.name}</Link>
          </p>
        </div>
      ))}
    </div>
